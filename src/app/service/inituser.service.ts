  ...
  fileuploads(url){
    // this.camera.getPicture(options).then((url) => {
      ...
      this.makeFileIntoBlob(url, name).then(imageData => {
        ...
        this.storageSer.uploadContent(imageData, name).then(async success => {
          // await this.loadingCtrl.dismiss();
          // this.createToast('image uploded', true, 'bottom', 2100);
          console.log('success', success);
          // this.api.updateUser(this.loggedInUser.id, { profile_img: success.url }).pipe(takeUntil(this.unSubscriber$)).subscribe(res => {
