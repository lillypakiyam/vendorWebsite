...
let TermsConditionsPage = class TermsConditionsPage {
    ...
    select(eve) {
        if (eve.detail.checked === true) {
            ...
            this.decision_Date = moment__WEBPACK_IMPORTED_MODULE_5__(currentDate).format('YYYY-MM-DD');
        }
        else {
            this.loggedInVendor.first_agree = false;
            const currentDate = new Date();
            this.effective_Date = moment__WEBPACK_IMPORTED_MODULE_5__(currentDate).format('YYYY-MM-DD');
        }
